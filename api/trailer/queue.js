...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // and never re-generate (and re-bill) a page that already has a
    // finished trailer — this route must be safe to call more than once.
    ...
    if (page.trailer_status === 'ready') {
      const { data: fresh } = await db.from('pages').select('trailer_url').eq('id', pageId).single();
      res.status(200).json({
        success: true,
        status: 'ready',
        trailerUrl: fresh && fresh.trailer_url,
        note: 'Trailer already generated for this page.',
      });
      return;
    }
